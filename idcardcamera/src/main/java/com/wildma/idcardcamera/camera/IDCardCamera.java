...
public class IDCardCamera {
    ...
    public final static String NUMBER_CHARACTERS             = "number_characters";//拍摄类型标记
   ...
   // private int numberCharacters;
     ...
     * @param
     */
    public void openCamera(int number) {
        Log.d("字符数量ID：", number + "");
        Activity activity = this.mActivity.get();
        Fragment fragment = this.mFragment.get();
        Intent intent = new Intent(activity, CameraActivity.class);
        intent.putExtra(NUMBER_CHARACTERS, number);
        if (fragment != null) {
            fragment.startActivityForResult(intent, number);
        } else {
            activity.startActivityForResult(intent, number);
        }
    }
